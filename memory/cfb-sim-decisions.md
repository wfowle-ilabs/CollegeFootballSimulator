...
- **Ratings/check mechanics (M1 spec, see `docs/mechanics.qmd`):** all ratings on **1–20** scale (BG3-pure, dice-dominant). Core attributes (6) and position skills are **independent stored axes** (skills NOT derived from attributes). A check **blends** them: `effectiveRating = 0.65·skill + 0.35·attribute`, then `modifier = floor((effectiveRating−10)/2)` (~−5..+5). Opposed check expressed as static DC: `d20 + attackerMod vs DC = 10 + defenderMod`; margin scales magnitude. Large rating gaps (≥6) grant **advantage/disadvantage** (favorite lever). Inside run = chain: push (trenches) → second level → open field. All numeric curves are first-pass, tuned in M9; structure is the M1 contract.
- **v1 scope:** Multiple seasons — start save, pick a team in a conference, sim full season incl. full CFP slate, then a lightweight offseason, then next season, indefinitely. FBS (FBS-IAA) only for v1.
- **Offseason/recruiting (v1):** real recruiting deferred to v2; v1 offseason = archive season → advance class years → graduate/depart seniors → **walk-on replenishment** via a deterministic, prestige-weighted `PlayerGenerator` (which also builds initial rosters). Recruiting later replaces the walk-on step behind the same seam.
- **Storage model:** save = thin **spine** (entity registry ID→sidecar, user team, current-season pointer, RNG state, schema version, manifest) + per-module **sidecars** (current-state, season-state, historical-state, media, stats-index). Cross-module refs are **by ID resolved via the spine registry — never object pointers**; sidecars never reference each other directly or co-load. Cross-cutting data (leaderboards/records) → dedicated stats-index updated incrementally from events. Heavy datasets promote to their own sidecar later without touching others. Atomic writes (temp+rename), version-stamped, one `ISaveModule`/`SaveManager` abstraction. Don't over-shard in v1.
